 ...
 * Used to gracefully skip tests when services aren't running
 */
export async function isServiceAvailable(url: string, request: any): Promise<boolean> {
  try {
...
export async function createOrganization(page: Page, name: string) {
  ...
  await page.click(
    '[data-testid="org-selector"], .org-selector, button:has-text("Create Organization")'
  );
...
export async function expectToast(page: Page, message: string | RegExp) {
  await expect(
    page.locator('.toast, [role="alert"], .notification').filter({ hasText: message })
  ).toBeVisible({
    timeout: 5000,
  });
}
