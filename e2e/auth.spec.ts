...
import { URLS, generateTestUser, login, waitForDashboard } from './utils'
...
test.describe('Authentication', () => {
  test.describe('Registration', () => {
    ...
    test('creates default organization after registration', async ({ page }) => {
      const user = generateTestUser()
      ...
      const orgIndicator = page.locator('text=' + user.name).or(
        page.locator('[data-testid="org-selector"]')
      ).or(
    ...
    test('shows error for duplicate email', async ({ page }) => {
      ...
      // Should show error
      await expect(page.locator('text=/already|exists|taken/i').or(
  ...
  test.describe('Login', () => {
    ...
    test('shows error for invalid credentials', async ({ page }) => {
      ...
      // Should show error
      await expect(page.locator('text=/invalid|incorrect|failed|error/i').or(
        page.locator('[role="alert"]')
      ).or(
        page.locator('.error')
      )).toBeVisible({ timeout: 5000 })
  ...
  test.describe('Protected Routes', () => {
    ...
    test('redirects to login when accessing services without auth', async ({ page }) => {
