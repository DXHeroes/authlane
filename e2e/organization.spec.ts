...
test.describe('Organization Management', () => {
  ...
  test.describe('Organization Creation', () => {
    test('default organization is created after registration', async ({ page, request }) => {
    ...
    test('can create a new organization', async ({ page, request }) => {
      ...
      const orgSelector = page
        .locator('[data-testid="org-selector"], button:has-text("Organization"), .org-selector')
        .first();
      await orgSelector.click();

      // Click "New Organization" or "Create Organization" option
      const createOrgOption = page
        .locator('text=New Organization')
        .or(page.locator('text=Create Organization'))
        .or(page.locator('[data-testid="create-org"]'));
  ...
  test.describe('Organization Switching', () => {
    test('can switch between organizations', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      if ((await createOrgOption.count()) > 0) {
        ...
        const orgList = page.locator(
          '[data-testid="org-list"] button, .org-item, [role="menuitem"]'
        );
  ...
  test.describe('Organization Settings', () => {
    test('can navigate to organization settings', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      const orgLink = page
        .locator('a[href*="organization"], text=Organization Settings, text=Organization')
        .first();
      await orgLink.click();
      ...
      await expect(
        page
          .locator('text=Organization Settings')
          .or(page.locator('h1:has-text("Organization")'))
          .or(page.locator('input[name="name"]'))
      ).toBeVisible({ timeout: 5000 });
    });

    test('can update organization name', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
  ...
  test.describe('Members Management', () => {
    test('can navigate to members page', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      if ((await membersLink.count()) > 0) {
        ...
        await expect(
          page
            ...
            .or(page.locator('h1:has-text("Members")'))
            .or(page.locator('table'))
        ).toBeVisible({ timeout: 5000 });
      }
    });

    test('shows current user as owner/member', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      if ((await membersLink.count()) > 0) {
        ...
        await expect(
          page
            ...
            .or(page.locator(`text=${user.name}`))
            .or(page.locator('text=Owner'))
        ).toBeVisible({ timeout: 5000 });
      }
    });

    test('can open invite member modal', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
    ...
    test('validates invite member form', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      if ((await membersLink.count()) > 0) {
        ...
        if ((await inviteButton.count()) > 0) {
          ...
          const submitButton = page.locator(
            'button[type="submit"], button:has-text("Send Invite")'
          );
