#!/bin/bash
# End-to-end OAuth test script for Authlane
# Tests GitHub OAuth flow: authorize → callback → credentials stored → token refresh
# Requires: API running, valid API key, GitHub OAuth App credentials
...
if [ -z "$GITHUB_CLIENT_ID" ] || [ -z "$GITHUB_CLIENT_SECRET" ]; then
  echo -e "${RED}❌ GitHub OAuth credentials required${NC}"
  echo "   Set GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET environment variables"
  echo "   See: docs/guides/github-oauth-setup.md"
  exit 1
fi

echo -e "${BLUE}🧪 Testing GitHub OAuth Flow...${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Step 2: Verify GitHub service exists
echo -e "${YELLOW}Step 2: Verify GitHub service configuration...${NC}"
service_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/services/github")

service_id=$(echo "$service_response" | jq -r '.data.id // empty')
auth_type=$(echo "$service_response" | jq -r '.data.auth_type // empty')

if [ "$service_id" = "github" ] && [ "$auth_type" = "oauth2" ]; then
  echo -e "${GREEN}✓ GitHub service configured correctly${NC}"
  ...
  echo -e "${RED}✗ GitHub service not properly configured${NC}"
...
authorize_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/github/authorize"
redirect_uri="$API_URL/api/v1/users/$TEST_USER_ID/connections/github/callback"

authorize_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$authorize_url?client_id=$GITHUB_CLIENT_ID&redirect_uri=$redirect_uri")
...
callback_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/github/callback"
callback_url+="?code=$auth_code&state=$state_param&client_id=$GITHUB_CLIENT_ID&client_secret=$GITHUB_CLIENT_SECRET"
...
connection_detail=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/github")
...
credentials_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/github/credentials")
...
# Step 9: Test credentials with GitHub API
echo -e "${YELLOW}Step 9: Test credentials with GitHub API...${NC}"
github_user_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://api.github.com/user")

github_login=$(echo "$github_user_response" | jq -r '.login // empty')

if [ -n "$github_login" ]; then
  echo -e "${GREEN}✓ Credentials valid - authenticated as: $github_login${NC}"
else
  echo -e "${RED}✗ GitHub API authentication failed${NC}"
  echo "$github_user_response" | jq .
  exit 1
fi
echo ""

# Step 10: Verify token refresh job (if Redis is configured)
...
if [ -n "$expires_at" ] && [ "$expires_at" != "null" ]; then
  ...
  echo -e "${YELLOW}⚠ Token has no expiration (GitHub tokens don't expire)${NC}"
fi
echo ""
...
health_check_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/github/health")
...
echo "  ✓ Tokens work with GitHub API"
