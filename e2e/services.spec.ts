...
test.describe('Services', () => {
  ...
  test.describe('Services List', () => {
    test('can navigate to services page', async ({ page, request }) => {
      ...
      // Should see services heading
      await expect(page.locator('h1:has-text("Services")')).toBeVisible({ timeout: 5000 });
    });

    test('displays services grouped by auth type', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      const sections = page
        ...
        .or(page.locator('text=Public API'))
        .or(page.locator('text=No Setup Required'));

      // At least one section should be visible
      await expect(sections.first()).toBeVisible({ timeout: 10000 });
    });

    test('displays service cards with auth type badges', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      // Should see service cards (using bg-card class from implementation)
      const serviceCards = page.locator('.bg-card');
      ...
      const badges = page
        ...
        .or(page.locator('text=oauth2'))
        .or(page.locator('text=api_key'))
        .or(page.locator('text=none'));
  ...
  test.describe('Service Toggle', () => {
    test('can enable a service via toggle', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      const toggle = page.locator(
        '[data-testid="service-toggle"], button[role="switch"], input[type="checkbox"], .toggle'
      );
      ...
      if ((await toggle.count()) > 0) {
        ...
        // Get initial state
        const _initialChecked = await firstToggle.isChecked().catch(() => false);
    ...
    test('toggle updates service status', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      const toggle = toggleContainer.locator(
        '[data-testid="service-toggle"], button[role="switch"], input[type="checkbox"]'
      );
  ...
  test.describe('Service Detail', () => {
    test('can navigate to service detail page', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      const serviceLink = page
        .locator('[data-testid="service-card"] a, .service-card a, .card a')
        .first();
    ...
    test('shows OAuth configuration for OAuth2 services', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      if ((await githubLink.count()) > 0) {
        ...
        await expect(
          page
            ...
            .or(page.locator('text=Client Secret'))
            .or(page.locator('text=Configuration'))
        ).toBeVisible({ timeout: 5000 });
      }
    });

    test('shows API info for public API services', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      const publicApiLink = page
        .locator('a[href*="jsonplaceholder"]')
        .or(page.locator('text=Public API'));
      ...
      if ((await publicApiLink.count()) > 0) {
        ...
        await expect(
          page
            .locator('text=API')
            .or(page.locator('text=Base URL'))
            .or(page.locator('text=Example'))
            .or(page.locator('text=No setup'))
        ).toBeVisible({ timeout: 5000 });
      }
    });

    test('shows API key config for API key services', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      const apiKeyServiceLink = page
        .locator('a[href*="openai"]')
        .or(page.locator('a[href*="anthropic"]'));
  ...
  test.describe('Service Stats', () => {
    test('displays service statistics', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.dashboard, request);
      test.skip(!isAvailable, 'Dashboard service not running');
      ...
      const statsSection = page
        .locator('text=Total')
        .or(page.locator('text=Enabled'))
        .or(page.locator('[data-testid="services-stats"]'));
