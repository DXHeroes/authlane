#!/bin/bash
# End-to-end OAuth test script for Authlane - Slack Integration
# Tests Slack OAuth flow: authorize → callback → credentials stored → token refresh
# Requires: API running, valid API key, Slack OAuth App credentials
...
if [ -z "$SLACK_CLIENT_ID" ] || [ -z "$SLACK_CLIENT_SECRET" ]; then
  echo -e "${RED}❌ Slack OAuth credentials required${NC}"
  echo "   Set SLACK_CLIENT_ID and SLACK_CLIENT_SECRET environment variables"
  echo "   See: https://api.slack.com/apps"
  exit 1
fi

echo -e "${BLUE}🧪 Testing Slack OAuth Flow...${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Step 2: Verify Slack service exists
echo -e "${YELLOW}Step 2: Verify Slack service configuration...${NC}"
service_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/services/slack")

service_id=$(echo "$service_response" | jq -r '.data.id // empty')
auth_type=$(echo "$service_response" | jq -r '.data.auth_type // empty')

if [ "$service_id" = "slack" ] && [ "$auth_type" = "oauth2" ]; then
  echo -e "${GREEN}✓ Slack service configured correctly${NC}"
  ...
  echo -e "${RED}✗ Slack service not properly configured${NC}"
...
authorize_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/slack/authorize"
redirect_uri="$API_URL/api/v1/users/$TEST_USER_ID/connections/slack/callback"

authorize_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$authorize_url?client_id=$SLACK_CLIENT_ID&redirect_uri=$redirect_uri")
...
echo ""
echo "2. Select a Slack workspace and authorize the application"
echo "3. Grant the requested permissions (chat:write, channels:read)"
...
callback_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/slack/callback"
callback_url+="?code=$auth_code&state=$state_param&client_id=$SLACK_CLIENT_ID&client_secret=$SLACK_CLIENT_SECRET"
...
connection_detail=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/slack")
...
credentials_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/slack/credentials")
...
# Step 9: Test credentials with Slack API
echo -e "${YELLOW}Step 9: Test credentials with Slack API...${NC}"
slack_auth_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://slack.com/api/auth.test")

auth_ok=$(echo "$slack_auth_response" | jq -r '.ok // false')
team_name=$(echo "$slack_auth_response" | jq -r '.team // empty')

if [ "$auth_ok" = "true" ]; then
  echo -e "${GREEN}✓ Credentials valid - connected to workspace: $team_name${NC}"
  echo "  User: $(echo "$slack_auth_response" | jq -r '.user')"
  echo "  Team ID: $(echo "$slack_auth_response" | jq -r '.team_id')"
else
  echo -e "${RED}✗ Slack API authentication failed${NC}"
  echo "$slack_auth_response" | jq .
  exit 1
fi
echo ""

# Step 10: Test listing channels with proper scopes
echo -e "${YELLOW}Step 10: Test Slack API permissions (channels:read)...${NC}"
slack_channels_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://slack.com/api/conversations.list?exclude_archived=true&limit=5")

channels_ok=$(echo "$slack_channels_response" | jq -r '.ok // false')

if [ "$channels_ok" = "true" ]; then
  channel_count=$(echo "$slack_channels_response" | jq -r '.channels | length')
  echo -e "${GREEN}✓ Successfully listed $channel_count channels${NC}"
  if [ "$channel_count" -gt 0 ]; then
    echo "  Example channel: $(echo "$slack_channels_response" | jq -r '.channels[0].name')"
  fi
else
  error_msg=$(echo "$slack_channels_response" | jq -r '.error // "unknown"')
  echo -e "${YELLOW}⚠ Could not list channels: $error_msg${NC}"
  echo "  This may be a scope issue - ensure 'channels:read' is granted"
fi
echo ""
...
health_check_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/slack/health")
...
# Summary
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✅ All Slack OAuth flow tests passed!${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "Summary:"
...
echo "  ✓ Tokens work with Slack API"
echo "  ✓ Required scopes verified (chat:write, channels:read)"
