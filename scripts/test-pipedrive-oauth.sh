#!/bin/bash
# End-to-end OAuth test script for Authlane - Pipedrive Integration
# Tests Pipedrive OAuth flow: authorize → callback → credentials stored → token refresh
# Requires: API running, valid API key, Pipedrive OAuth App credentials
...
if [ -z "$PIPEDRIVE_CLIENT_ID" ] || [ -z "$PIPEDRIVE_CLIENT_SECRET" ]; then
  echo -e "${RED}❌ Pipedrive OAuth credentials required${NC}"
  echo "   Set PIPEDRIVE_CLIENT_ID and PIPEDRIVE_CLIENT_SECRET environment variables"
  echo "   See: https://pipedrive.readme.io/docs/marketplace-oauth-authorization"
  exit 1
fi

echo -e "${BLUE}🧪 Testing Pipedrive OAuth Flow...${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Step 2: Verify Pipedrive service exists
echo -e "${YELLOW}Step 2: Verify Pipedrive service configuration...${NC}"
service_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/services/pipedrive")

service_id=$(echo "$service_response" | jq -r '.data.id // empty')
auth_type=$(echo "$service_response" | jq -r '.data.auth_type // empty')

if [ "$service_id" = "pipedrive" ] && [ "$auth_type" = "oauth2" ]; then
  echo -e "${GREEN}✓ Pipedrive service configured correctly${NC}"
  ...
  echo -e "${RED}✗ Pipedrive service not properly configured${NC}"
...
authorize_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/pipedrive/authorize"
redirect_uri="$API_URL/api/v1/users/$TEST_USER_ID/connections/pipedrive/callback"

authorize_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$authorize_url?client_id=$PIPEDRIVE_CLIENT_ID&redirect_uri=$redirect_uri")
...
echo ""
echo "2. Sign in to your Pipedrive account (or select a company account)"
echo "3. Grant the requested permissions:"
echo "   - deals:read, deals:write"
echo "   - contacts:read, contacts:write"
echo "   - persons:read, persons:write"
...
callback_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/pipedrive/callback"
callback_url+="?code=$auth_code&state=$state_param&client_id=$PIPEDRIVE_CLIENT_ID&client_secret=$PIPEDRIVE_CLIENT_SECRET"
...
connection_detail=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/pipedrive")
...
credentials_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/pipedrive/credentials")

access_token=$(echo "$credentials_response" | jq -r '.data.access_token // empty')
api_domain=$(echo "$credentials_response" | jq -r '.data.api_domain // empty')
...
# Step 9: Test credentials with Pipedrive API
echo -e "${YELLOW}Step 9: Test credentials with Pipedrive API...${NC}"

# Pipedrive uses company-specific API domains returned in OAuth
if [ -z "$api_domain" ]; then
  echo -e "${YELLOW}⚠ API domain not found, attempting to use generic domain${NC}"
  api_domain="api.pipedrive.com"
fi

pipedrive_user_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://$api_domain/v1/users/me")

auth_ok=$(echo "$pipedrive_user_response" | jq -r '.success // false')

if [ "$auth_ok" = "true" ]; then
  echo -e "${GREEN}✓ Credentials valid - connected to Pipedrive${NC}"
  echo "  User: $(echo "$pipedrive_user_response" | jq -r '.data.name')"
  echo "  Company: $(echo "$pipedrive_user_response" | jq -r '.data.company_name')"
  echo "  User ID: $(echo "$pipedrive_user_response" | jq -r '.data.id')"
else
  echo -e "${RED}✗ Pipedrive API authentication failed${NC}"
  echo "$pipedrive_user_response" | jq .
  exit 1
fi
echo ""

# Step 10: Test Pipedrive API permissions (deals)
echo -e "${YELLOW}Step 10: Test Pipedrive API permissions (list deals)...${NC}"
pipedrive_deals_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://$api_domain/v1/deals?limit=5")

deals_ok=$(echo "$pipedrive_deals_response" | jq -r '.success // false')

if [ "$deals_ok" = "true" ]; then
  deal_count=$(echo "$pipedrive_deals_response" | jq -r '.data | length // 0')
  echo -e "${GREEN}✓ Successfully accessed deals endpoint${NC}"
  echo "  Returned $deal_count deals"
  if [ "$deal_count" -gt 0 ]; then
    echo "  Example deal ID: $(echo "$pipedrive_deals_response" | jq -r '.data[0].id')"
    echo "  Example deal title: $(echo "$pipedrive_deals_response" | jq -r '.data[0].title')"
  fi
else
  error_msg=$(echo "$pipedrive_deals_response" | jq -r '.error // "unknown"')
  echo -e "${YELLOW}⚠ Could not list deals: $error_msg${NC}"
  echo "  This may be a scope issue - ensure 'deals:read' is granted"
fi
echo ""
...
health_check_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/pipedrive/health")
...
# Summary
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✅ All Pipedrive OAuth flow tests passed!${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "Summary:"
...
echo "  ✓ Tokens work with Pipedrive API"
echo "  ✓ Required scopes verified (deals, contacts, persons)"
