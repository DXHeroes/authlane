...
Get `credential` from the server-side Authlane credentials endpoint. The adapter calls Discord directly; tool inputs and provider responses do not pass through Authlane.
