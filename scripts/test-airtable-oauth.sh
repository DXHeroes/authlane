#!/bin/bash
# End-to-end OAuth test script for Authlane - Airtable Integration
# Tests Airtable OAuth flow: authorize → callback → credentials stored → token refresh
# Requires: API running, valid API key, Airtable OAuth App credentials
...
if [ -z "$AIRTABLE_CLIENT_ID" ] || [ -z "$AIRTABLE_CLIENT_SECRET" ]; then
  echo -e "${RED}❌ Airtable OAuth credentials required${NC}"
  echo "   Set AIRTABLE_CLIENT_ID and AIRTABLE_CLIENT_SECRET environment variables"
  echo "   See: https://airtable.com/create/oauth"
  exit 1
fi

echo -e "${BLUE}🧪 Testing Airtable OAuth Flow...${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Step 2: Verify Airtable service exists
echo -e "${YELLOW}Step 2: Verify Airtable service configuration...${NC}"
service_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/services/airtable")

service_id=$(echo "$service_response" | jq -r '.data.id // empty')
auth_type=$(echo "$service_response" | jq -r '.data.auth_type // empty')

if [ "$service_id" = "airtable" ] && [ "$auth_type" = "oauth2" ]; then
  echo -e "${GREEN}✓ Airtable service configured correctly${NC}"
  ...
  echo -e "${RED}✗ Airtable service not properly configured${NC}"
...
authorize_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/airtable/authorize"
redirect_uri="$API_URL/api/v1/users/$TEST_USER_ID/connections/airtable/callback"

authorize_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$authorize_url?client_id=$AIRTABLE_CLIENT_ID&redirect_uri=$redirect_uri")
...
echo ""
echo "2. Sign in to Airtable and authorize the application"
echo "3. Grant the requested permissions (data.records:read, data.records:write, schema.bases:read)"
...
callback_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/airtable/callback"
callback_url+="?code=$auth_code&state=$state_param&client_id=$AIRTABLE_CLIENT_ID&client_secret=$AIRTABLE_CLIENT_SECRET"
...
connection_detail=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/airtable")
...
credentials_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/airtable/credentials")
...
# Step 9: Test credentials with Airtable API
echo -e "${YELLOW}Step 9: Test credentials with Airtable API...${NC}"
airtable_whoami_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://api.airtable.com/v0/meta/whoami")

whoami_id=$(echo "$airtable_whoami_response" | jq -r '.id // empty')

if [ -n "$whoami_id" ]; then
  echo -e "${GREEN}✓ Credentials valid - authenticated user${NC}"
  echo "  User ID: $whoami_id"
  echo "  Email: $(echo "$airtable_whoami_response" | jq -r '.email // "N/A"')"
else
  echo -e "${RED}✗ Airtable API authentication failed${NC}"
  echo "$airtable_whoami_response" | jq .
  exit 1
fi
echo ""

# Step 10: Test listing bases with proper scopes
echo -e "${YELLOW}Step 10: Test Airtable API permissions (schema.bases:read)...${NC}"
airtable_bases_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://api.airtable.com/v0/meta/bases")

bases=$(echo "$airtable_bases_response" | jq -r '.bases // empty')

if [ -n "$bases" ]; then
  base_count=$(echo "$airtable_bases_response" | jq -r '.bases | length')
  echo -e "${GREEN}✓ Successfully listed $base_count bases${NC}"
  if [ "$base_count" -gt 0 ]; then
    echo "  Example base: $(echo "$airtable_bases_response" | jq -r '.bases[0].name')"
    echo "  Base ID: $(echo "$airtable_bases_response" | jq -r '.bases[0].id')"
  fi
else
  error=$(echo "$airtable_bases_response" | jq -r '.error // empty')
  if [ -n "$error" ]; then
    error_type=$(echo "$error" | jq -r '.type // "unknown"')
    error_msg=$(echo "$error" | jq -r '.message // "unknown"')
    echo -e "${YELLOW}⚠ Could not list bases: $error_type - $error_msg${NC}"
    echo "  This may be a scope issue - ensure 'schema.bases:read' is granted"
  else
    echo -e "${YELLOW}⚠ No bases found or unable to access${NC}"
  fi
fi
echo ""
...
health_check_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/airtable/health")
...
# Summary
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✅ All Airtable OAuth flow tests passed!${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "Summary:"
...
echo "  ✓ Tokens work with Airtable API"
echo "  ✓ Required scopes verified (data.records:read, data.records:write, schema.bases:read)"
