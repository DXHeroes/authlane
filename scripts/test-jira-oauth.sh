#!/bin/bash
# End-to-end OAuth test script for Jira integration
# Tests Jira OAuth flow: authorize → callback → credentials stored → token refresh
# Requires: API running, valid API key, Jira OAuth App credentials
...
if [ -z "$JIRA_CLIENT_ID" ] || [ -z "$JIRA_CLIENT_SECRET" ]; then
  echo -e "${RED}❌ Jira OAuth credentials required${NC}"
  echo "   Set JIRA_CLIENT_ID and JIRA_CLIENT_SECRET environment variables"
  echo "   See: https://developer.atlassian.com/cloud/jira/platform/oauth-2-3lo-apps/"
  exit 1
fi

echo -e "${BLUE}🧪 Testing Jira OAuth Flow...${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Step 2: Verify Jira service exists
echo -e "${YELLOW}Step 2: Verify Jira service configuration...${NC}"
service_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/services/jira")

service_id=$(echo "$service_response" | jq -r '.data.id // empty')
auth_type=$(echo "$service_response" | jq -r '.data.auth_type // empty')

if [ "$service_id" = "jira" ] && [ "$auth_type" = "oauth2" ]; then
  echo -e "${GREEN}✓ Jira service configured correctly${NC}"
  ...
  echo -e "${RED}✗ Jira service not properly configured${NC}"
...
authorize_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/jira/authorize"
redirect_uri="$API_URL/api/v1/users/$TEST_USER_ID/connections/jira/callback"

authorize_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$authorize_url?client_id=$JIRA_CLIENT_ID&redirect_uri=$redirect_uri")
...
callback_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/jira/callback"
callback_url+="?code=$auth_code&state=$state_param&client_id=$JIRA_CLIENT_ID&client_secret=$JIRA_CLIENT_SECRET"
...
connection_detail=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/jira")
...
credentials_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/jira/credentials")
...
# Step 9: Get accessible resources (Jira cloud instances)
echo -e "${YELLOW}Step 9: Test credentials with Jira API...${NC}"
resources_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://api.atlassian.com/oauth/token/accessible-resources")

resource_count=$(echo "$resources_response" | jq '. | length')

if [ "$resource_count" -gt 0 ]; then
  echo -e "${GREEN}✓ Credentials valid - found $resource_count accessible resource(s)${NC}"
  echo "$resources_response" | jq -r '.[] | "  - \(.name) (ID: \(.id))"'
else
  echo -e "${RED}✗ Jira API authentication failed${NC}"
  echo "$resources_response" | jq .
  exit 1
fi
echo ""

# Step 10: Verify token refresh job (Jira tokens expire)
...
health_check_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/jira/health")
...
# Summary
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✅ All Jira OAuth flow tests passed!${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "Summary:"
...
echo "  ✓ Tokens work with Jira API"
