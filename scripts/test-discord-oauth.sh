#!/bin/bash
# End-to-end OAuth test script for Authlane - Discord Integration
# Tests Discord OAuth flow: authorize → callback → credentials stored → token refresh
# Requires: API running, valid API key, Discord OAuth App credentials
...
if [ -z "$DISCORD_CLIENT_ID" ] || [ -z "$DISCORD_CLIENT_SECRET" ]; then
  echo -e "${RED}❌ Discord OAuth credentials required${NC}"
  echo "   Set DISCORD_CLIENT_ID and DISCORD_CLIENT_SECRET environment variables"
...
# Step 2: Verify Discord service exists
echo -e "${YELLOW}Step 2: Verify Discord service configuration...${NC}"
service_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/services/discord")

service_id=$(echo "$service_response" | jq -r '.data.id // empty')
auth_type=$(echo "$service_response" | jq -r '.data.auth_type // empty')

if [ "$service_id" = "discord" ] && [ "$auth_type" = "oauth2" ]; then
  echo -e "${GREEN}✓ Discord service configured correctly${NC}"
  ...
  echo -e "${RED}✗ Discord service not properly configured${NC}"
...
authorize_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/discord/authorize"
redirect_uri="$API_URL/api/v1/users/$TEST_USER_ID/connections/discord/callback"

authorize_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$authorize_url?client_id=$DISCORD_CLIENT_ID&redirect_uri=$redirect_uri")
...
echo ""
echo "2. Select a Discord server and authorize the application"
echo "3. Grant the requested permissions (bot, messages.write)"
...
callback_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/discord/callback"
callback_url+="?code=$auth_code&state=$state_param&client_id=$DISCORD_CLIENT_ID&client_secret=$DISCORD_CLIENT_SECRET"
...
connection_detail=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/discord")
...
credentials_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/discord/credentials")
...
# Step 9: Test credentials with Discord API
echo -e "${YELLOW}Step 9: Test credentials with Discord API...${NC}"
discord_auth_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://discord.com/api/v10/users/@me")

user_id=$(echo "$discord_auth_response" | jq -r '.id // empty')
username=$(echo "$discord_auth_response" | jq -r '.username // empty')

if [ -n "$user_id" ] && [ -n "$username" ]; then
  echo -e "${GREEN}✓ Credentials valid - authenticated as: $username#$(echo "$discord_auth_response" | jq -r '.discriminator')${NC}"
  echo "  User ID: $user_id"
  echo "  Email: $(echo "$discord_auth_response" | jq -r '.email // "N/A"')"
else
  echo -e "${RED}✗ Discord API authentication failed${NC}"
  echo "$discord_auth_response" | jq .
  exit 1
fi
echo ""

# Step 10: Test listing guilds with proper scopes
echo -e "${YELLOW}Step 10: Test Discord API permissions (guilds)...${NC}"
discord_guilds_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://discord.com/api/v10/users/@me/guilds")

if echo "$discord_guilds_response" | jq -e 'type == "array"' > /dev/null 2>&1; then
  guild_count=$(echo "$discord_guilds_response" | jq 'length')
  echo -e "${GREEN}✓ Successfully listed $guild_count guilds${NC}"
  if [ "$guild_count" -gt 0 ]; then
    echo "  Example guild: $(echo "$discord_guilds_response" | jq -r '.[0].name')"
  fi
else
  error_msg=$(echo "$discord_guilds_response" | jq -r '.message // "unknown"')
  echo -e "${YELLOW}⚠ Could not list guilds: $error_msg${NC}"
  echo "  This may be a scope issue - ensure 'guilds' is granted"
fi
echo ""
...
health_check_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/discord/health")
...
# Summary
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✅ All Discord OAuth flow tests passed!${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "Summary:"
...
echo "  ✓ Tokens work with Discord API"
echo "  ✓ Required scopes verified (bot, messages.write)"
