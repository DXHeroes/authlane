#!/bin/bash
# End-to-end OAuth test script for Authlane - HubSpot Integration
# Tests HubSpot OAuth flow: authorize → callback → credentials stored → token refresh
# Requires: API running, valid API key, HubSpot OAuth App credentials
...
if [ -z "$HUBSPOT_CLIENT_ID" ] || [ -z "$HUBSPOT_CLIENT_SECRET" ]; then
  echo -e "${RED}❌ HubSpot OAuth credentials required${NC}"
  echo "   Set HUBSPOT_CLIENT_ID and HUBSPOT_CLIENT_SECRET environment variables"
  echo "   See: https://developers.hubspot.com/docs/api/creating-an-app"
  exit 1
fi

echo -e "${BLUE}🧪 Testing HubSpot OAuth Flow...${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Step 2: Verify HubSpot service exists
echo -e "${YELLOW}Step 2: Verify HubSpot service configuration...${NC}"
service_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/services/hubspot")

service_id=$(echo "$service_response" | jq -r '.data.id // empty')
auth_type=$(echo "$service_response" | jq -r '.data.auth_type // empty')

if [ "$service_id" = "hubspot" ] && [ "$auth_type" = "oauth2" ]; then
  echo -e "${GREEN}✓ HubSpot service configured correctly${NC}"
  ...
  echo -e "${RED}✗ HubSpot service not properly configured${NC}"
...
authorize_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/hubspot/authorize"
redirect_uri="$API_URL/api/v1/users/$TEST_USER_ID/connections/hubspot/callback"

authorize_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$authorize_url?client_id=$HUBSPOT_CLIENT_ID&redirect_uri=$redirect_uri")
...
echo ""
echo "2. Sign in to your HubSpot account (or select an account)"
...
callback_url="$API_URL/api/v1/users/$TEST_USER_ID/connections/hubspot/callback"
callback_url+="?code=$auth_code&state=$state_param&client_id=$HUBSPOT_CLIENT_ID&client_secret=$HUBSPOT_CLIENT_SECRET"
...
connection_detail=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/hubspot")
...
credentials_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/hubspot/credentials")
...
# Step 9: Test credentials with HubSpot API
echo -e "${YELLOW}Step 9: Test credentials with HubSpot API...${NC}"
hubspot_auth_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://api.hubapi.com/oauth/v1/access-tokens/$access_token")

auth_ok=$(echo "$hubspot_auth_response" | jq -r 'has("hub_id")')

if [ "$auth_ok" = "true" ]; then
  echo -e "${GREEN}✓ Credentials valid - connected to HubSpot${NC}"
  echo "  Hub ID: $(echo "$hubspot_auth_response" | jq -r '.hub_id')"
  echo "  User ID: $(echo "$hubspot_auth_response" | jq -r '.user_id')"
else
  echo -e "${RED}✗ HubSpot API authentication failed${NC}"
  echo "$hubspot_auth_response" | jq .
  exit 1
fi
echo ""

# Step 10: Test HubSpot API permissions (contacts)
echo -e "${YELLOW}Step 10: Test HubSpot API permissions (list contacts)...${NC}"
hubspot_contacts_response=$(curl -s -H "Authorization: Bearer $access_token" \
  "https://api.hubapi.com/crm/v3/objects/contacts?limit=5")

contacts_ok=$(echo "$hubspot_contacts_response" | jq -r 'has("results")')

if [ "$contacts_ok" = "true" ]; then
  contact_count=$(echo "$hubspot_contacts_response" | jq -r '.results | length')
  echo -e "${GREEN}✓ Successfully accessed contacts endpoint${NC}"
  echo "  Returned $contact_count contacts"
  if [ "$contact_count" -gt 0 ]; then
    echo "  Example contact ID: $(echo "$hubspot_contacts_response" | jq -r '.results[0].id')"
  fi
else
  error_msg=$(echo "$hubspot_contacts_response" | jq -r '.message // "unknown"')
  echo -e "${YELLOW}⚠ Could not list contacts: $error_msg${NC}"
  echo "  This may be a scope issue - ensure 'crm.objects.contacts.read' is granted"
fi
echo ""
...
health_check_response=$(curl -s -H "Authorization: Bearer $API_KEY" \
  "$API_URL/api/v1/users/$TEST_USER_ID/connections/hubspot/health")
...
# Summary
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${GREEN}✅ All HubSpot OAuth flow tests passed!${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo "Summary:"
...
echo "  ✓ Tokens work with HubSpot API"
echo "  ✓ Required scopes verified (crm.objects.contacts, crm.objects.deals)"
