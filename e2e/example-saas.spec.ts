...
test.describe('Example SaaS Application', () => {
  test.describe('Home Page', () => {
    test('loads the home page', async ({ page, request }) => {
      ...
      await expect(
        page.locator('text=Example SaaS').or(page.locator('h1')).or(page.locator('text=Authlane'))
      ).toBeVisible({ timeout: 10000 });
    });

    test('displays connection status component', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.exampleSaas, request);
      test.skip(!isAvailable, 'Example SaaS service not running');
      ...
      await expect(
        page
          ...
          .or(page.locator('text=Integration'))
  ...
  test.describe('GitHub Demo Page', () => {
    test('can navigate to GitHub demo page', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.exampleSaas, request);
      test.skip(!isAvailable, 'Example SaaS service not running');
      ...
      if ((await githubLink.count()) > 0) {
        ...
        await expect(
          page
            ...
            .or(page.locator('h1:has-text("GitHub")'))
        ).toBeVisible({ timeout: 5000 });
      }
    });

    test('shows connect button or OAuth instructions', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.exampleSaas, request);
      test.skip(!isAvailable, 'Example SaaS service not running');
      ...
      await expect(
        page
          ...
          .or(page.locator('text=Connect'))
          .or(page.locator('text=OAuth'))
          .or(page.locator('text=Configure'))
          .or(page.locator('text=Not Connected'))
      ).toBeVisible({ timeout: 5000 });
    });
  });

  test.describe('Posts Page (Public API)', () => {
    test('can navigate to posts page', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.exampleSaas, request);
      test.skip(!isAvailable, 'Example SaaS service not running');
      ...
      if ((await postsLink.count()) > 0) {
        ...
        await expect(page.locator('text=Posts').or(page.locator('h1'))).toBeVisible({
          timeout: 5000,
        });
      }
    });

    test('fetches and displays posts from JSONPlaceholder', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.exampleSaas, request);
      test.skip(!isAvailable, 'Example SaaS service not running');
      ...
      const postsContent = page
        .locator('[data-testid="posts-list"]')
        .or(page.locator('.post'))
      ...
      await expect(
        postsContent.first().or(page.locator('text=Loading')).or(page.locator('text=No posts'))
      ).toBeVisible({ timeout: 10000 });
    });

    test('shows post details (title, body)', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.exampleSaas, request);
      test.skip(!isAvailable, 'Example SaaS service not running');
      ...
      const _postTitles = page.locator('.post-title, h2, h3, [data-testid="post-title"]');
  ...
  test.describe('Navigation', () => {
    test('can navigate between pages', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.exampleSaas, request);
      test.skip(!isAvailable, 'Example SaaS service not running');
  ...
  test.describe('Authlane Integration', () => {
    test('displays Authlane connection information', async ({ page, request }) => {
      const isAvailable = await isServiceAvailable(URLS.exampleSaas, request);
      test.skip(!isAvailable, 'Example SaaS service not running');
      ...
      await expect(
        page
          .locator('text=Authlane')
          .or(page.locator('text=Connection'))
          .or(page.locator('text=Integration'))
          .or(page.locator('[data-testid="authlane-status"]'))
      ).toBeVisible({ timeout: 10000 });
    });
  });
});
