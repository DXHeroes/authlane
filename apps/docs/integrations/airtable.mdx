---
title: Airtable
description: Use the local Airtable adapter with Authlane credentials
---
...
Get `credential` from the server-side Authlane credentials endpoint. The adapter calls Airtable directly; tool inputs and provider responses do not pass through Authlane.
