---
title: Linear
description: Use the local Linear adapter with Authlane credentials
---
...
Get `credential` from the server-side Authlane credentials endpoint. The adapter calls Linear directly; tool inputs and provider responses do not pass through Authlane.
